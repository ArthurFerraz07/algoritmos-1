# frozen_string_literal: true

# Algorithms who uses dfs algorithm to afirm something
module DFS
  # Find cycle in an undirected graph
  class FindCycle
    include AppHelper

    def initialize(graph)
      @graph = graph
      @reached_vertices_arr = [false] * @graph.vertices_size
      @reached_vertices_count = 0
      @total_rounds = 0
      @vertices_adjacency_lists = @graph.vertices.map(&:adjacency_list_ids)
      @reached_edges_matrix = @graph.vertices_size.times.map { [false] * @graph.vertices_size }
    end

    def call(initial_vertice_id, current_stack = [])
      return response(true) if reached_vertice?(initial_vertice_id)

      current_stack << initial_vertice_id
      reach_vertice(initial_vertice_id)

      get_vertice_adjacency_list_ids(initial_vertice_id).each do |vertice_id|
        @total_rounds += 1

        if reached_vertice?(vertice_id) && !reached_edge?(initial_vertice_id, vertice_id)
          raise "cycle found. cycle: [ #{current_stack.join(', ')} ]"
        end

        reach_edge(initial_vertice_id, vertice_id)
        res = call(vertice_id, current_stack)
        raise res[:error] if res[:error]
      end
      current_stack.pop

      response(true)
    rescue StandardError => e
      response(false, e)
    end

    private

    def get_vertice_adjacency_list_ids(vertice_id)
      # binding.pry
      @vertices_adjacency_lists[id_to_index(vertice_id)]
      # @graph.get_vertice_by_id(vertice_id).adjacency_list_ids
    end

    def reach_edge(vertice_id_a, vertice_id_b)
      @reached_edges_matrix[id_to_index(vertice_id_a)][id_to_index(vertice_id_b)] = true
      @reached_edges_matrix[id_to_index(vertice_id_b)][id_to_index(vertice_id_a)] = true
    end

    def reach_vertice(vertice_id)
      return if reached_vertice?(vertice_id)

      @reached_vertices_arr[id_to_index(vertice_id)] = true
      @reached_vertices_count += 1
    end

    def response(finished, error = nil)
      {
        finished: finished,
        reached_vertices_arr: @reached_vertices_arr,
        reached_vertices_count: @reached_vertices_count,
        rounds: @total_rounds,
        error: error&.message
      }
    end

    def reached_vertice?(vertice_id)
      @reached_vertices_arr[id_to_index(vertice_id)]
    end

    def reached_edge?(vertice_id_a, vertice_id_b)
      @reached_edges_matrix[id_to_index(vertice_id_a)][id_to_index(vertice_id_b)]
    end
  end
end
